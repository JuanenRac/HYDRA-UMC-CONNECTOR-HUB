...
printf '\n*******************************************************************************\n'
printf '%s\n' "* HYDRA-UMC-CONNECTOR-HUB - run.sh"
printf '%s\n' "* Mode      : RUN WORKFLOW"
printf '%s\n' "* Author    : JuanenRac (Electro Hobby 3D)"
printf '%s\n' "* Email     : electrohobby3d@gmail.com"
printf '%s\n' "* Copyright : (C) 2026 JuanenRac"
...
# Runs HYDRA-UMC-CONNECTOR-HUB. Run ./build.sh first.
#
# Usage:
#   ./run.sh                              - real demo: validates the ten
#                                            real fixtures under fixtures/
#   ./run.sh validate <file> [<file> ...] - validate specific manifest(s)
# This delivery is CLI-only (no GUI, no network, no hardware).
...
if [ "$#" -eq 0 ]; then
    printf '%s\n' "No arguments given - validating the ten real fixtures under fixtures/."
    python -m hydra_umc_connector_hub.cli validate fixtures/*.json
    status=$?
else
    python -m hydra_umc_connector_hub.cli "$@"
    status=$?
fi
exit "$status"
